    ...
    public static async Task SeedAsync(IServiceProvider services)
    {
        ...
        if (!await db.GroupTypes.AnyAsync())
        {
            ...
            db.GroupTypes.AddRange(
                new ChurchManager.Domain.Groups.GroupType { OrganizationId = org.Id, Name = "Small Group",      Color = "#4CAF50", IconName = "groups" },
                new ChurchManager.Domain.Groups.GroupType { OrganizationId = org.Id, Name = "Ministry Team",    Color = "#2196F3", IconName = "volunteer_activism" },
                new ChurchManager.Domain.Groups.GroupType { OrganizationId = org.Id, Name = "Bible Study",      Color = "#FF9800", IconName = "menu_book" },
                new ChurchManager.Domain.Groups.GroupType { OrganizationId = org.Id, Name = "Prayer Group",     Color = "#9C27B0", IconName = "self_improvement" },
                new ChurchManager.Domain.Groups.GroupType { OrganizationId = org.Id, Name = "Youth",            Color = "#F44336", IconName = "sports_soccer" },
                new ChurchManager.Domain.Groups.GroupType { OrganizationId = org.Id, Name = "Children",         Color = "#FF5722", IconName = "child_care" },
                new ChurchManager.Domain.Groups.GroupType { OrganizationId = org.Id, Name = "Leadership Team",  Color = "#607D8B", IconName = "star" }
            );
            await db.SaveChangesAsync();
        }

        // Seed default admin user
        const string adminEmail = "admin@churchmanager.local";
